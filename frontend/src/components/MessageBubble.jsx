...
const MessageBubble = ({role, content, images}) => {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`w-fit max-w-[92vw] md:max-w-[72%] px-4 py-2.5 rounded-2xl wrap-break-words overflow-hidden leading-relaxed 
      ${
          isUser
...

        {
          images?.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-4">
              {images.map((img, i)=>(
                <img key={i} src={img} loading="lazy" onError={(e)=>e.currentTarget.remove()}
        ...
        components={{
          ...
          p: ({children})=>(
            <p className='mb-3 whitespace-pre-wrap wrap-break-words'>{children}</p>
          ),
          ul: ({children})=>(
          ...
          img: ({src}) => {
            if(!src) return null;
            return (
              <img src={src} loading="lazy" onError={(e)=>e.currentTarget.remove()}
