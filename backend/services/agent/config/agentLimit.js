import redis from "../../../shared/redis/redis.js"

const Limits = {
    chat: 20, // 20 chats per minute
    coding: 5,
    pdf: 5,
    ppt: 5,
    image: 5,
    search: 5
}

export const checkAgentLimit = async (userId, agent) => {
    const max = Limits[agent] || Limits[chat]
    const key = `rate:${userId}:${agent}`
    const count = await redis.incr(key) // increment the key so server can know how much it fetches per minute
    if(count==1) {
        await redis.expire(key, 60)
    }
    const ttl = await redis.ttl(key)
    if(count > max) {
        const minutes = Math.floor(ttl/60)
        const seconds = (ttl%60) // after 60 start with zero
        const time = minutes > 0 ? `${minutes}m : ${seconds}s` : `${seconds}s`
        const error = new Error(`Rate limit exceeded for ${agent}.`)
        error.status = 429
        error.data = {
            success: false,
            agent,
            limit: max,
            remainingTime: ttl,
            retryAfter: time,
            message: `You have reached the ${agent} limit (${max} requests/minute). Try again in ${time}.`
        }
        throw error
    }
    return {
        remaining: max - count,
        limit: max
    }
}